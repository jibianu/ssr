...
interface PageExpectations {
  path: string;
  pageType: 'home' | 'course' | 'event' | 'static';
  expectedSchemas: ('Organization' | 'Course' | 'Event' | 'WebSite')[];
}
...
// These should be adjusted based on actual routes in your application
...
function fetchHtml(url: string): Promise<string> {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Fallback to curl if URL parsing fails or on certain systems
      try {
        const curlOutput = execSync(`curl -s --max-time 10 "${url}"`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] });
        resolve(curlOutput);
      } catch (curlError) {
        reject(new Error(`Failed to fetch URL: ${error instanceof Error ? error.message : String(error)}`));
      }
    }
  });
}
 ...
 */
function verifyMetaTags(html: string, pageType: PageExpectations['pageType'], expectedSchemas: string[]): VerificationResult['checks'] {
  const checks = {
 ...
 */
async function verifyPage(page: PageExpectations): Promise<VerificationResult> {
