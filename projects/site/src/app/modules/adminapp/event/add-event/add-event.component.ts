  ...
  socailMedias=[
    ...
    formInit() {
        this.eventForm = this.formBuilder.group({
            ...
            isPublished: [false], // When true, event appears on public /events page
    ...
    getEventById(id) {
        ...
        this.subscription.add(this.appService.getEventById(id).subscribe((res: any) => {
            if (res && this.eventForm) {
                ...
                const isPublishedValue = res.isPublished !== undefined ? res.isPublished : (res.IsPublished !== undefined ? res.IsPublished : false);
                ...
                const isPublishedBool = isPublishedValue === true || isPublishedValue === 'true' || isPublishedValue === 1 || isPublishedValue === '1';
                ...
                const isPublishedControl = this.eventForm.get('isPublished');
                if (showOnDashboardControl) showOnDashboardControl.setValue(showOnDashboardBool, { emitEvent: false });
                if (registrationCompletedControl) registrationCompletedControl.setValue(registrationCompletedBool, { emitEvent: false });
                if (isPublishedControl) isPublishedControl.setValue(isPublishedBool, { emitEvent: false });
    ...
    onSubmit() {
        ...
        if (this.eventId && !this.isNew) {
            ...
            const isPublishedValue = isPublishedControl ? isPublishedControl.value : false;
    ...
    updateFormWithServerValue(serverValue: any) {
        try {
            ...
            const isPublishedValue = serverValue.isPublished !== undefined ? serverValue.isPublished : (serverValue.IsPublished !== undefined ? serverValue.IsPublished : false);
    ...
    reloadEventDataWithRetry(maxRetries: number = 3, initialDelay: number = 200) {
        ...
        const attemptReload = () => {
            ...
            const performReload = () => {
                ...
                const cacheBustUrl = `${this.apiUrl}api/events/${this.eventId}?_refresh=${Date.now()}`;
                this.subscription.add(
