...
export class CacheInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    // ✅ Do not cache public events list – must reflect publish/unpublish immediately
    if (request.url.includes('api/events/dashboard')) {
      return next.handle(request);
    }
  ...
  private getBasePath(url: string): string {
    ...
    if (eventsIndex >= 1 && pathParts[eventsIndex - 1] === 'api') {
      return pathParts.slice(0, eventsIndex + 1).join('/');
    }
