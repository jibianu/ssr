...
export class NotificationListComponent implements OnInit {
  ...
  load(): void {
    ...
    if (this.filterTitle?.trim()) params.title = this.filterTitle.trim();
    if (this.filterType?.trim()) params.type = this.filterType.trim();
    if (this.filterSendTo?.trim()) params.sendTo = this.filterSendTo.trim();
    if (this.filterFromDate?.trim()) params.fromDate = this.filterFromDate.trim();
    if (this.filterToDate?.trim()) params.toDate = this.filterToDate.trim();
    ...
    this.appService.getNotificationHistory(params).subscribe({
      ...
      error: (err) => {
        ...
        this.loadError = err?.error?.message || err?.message || 'Could not load notifications.';
      }
    });
  }
  ...
  onUpdate(): void {
    ...
    this.appService.updateNotificationHistory(id, body).subscribe({
      next: () => {
        ...
        this.load();
        this.viewDetail = { ...this.viewDetail, ...value };
      },
      error: (err) => {
        this.panelSaving = false;
